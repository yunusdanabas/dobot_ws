...
def _viz_process(queue) -> None:
    ...
    class _DobotVizWindow(QMainWindow):
        def __init__(self, q):
            ...
            # ---- Left: Top View (XY) ----------------------------------------
            self._plot_xy = pg.PlotWidget(title="Top View (XY)  —  white=hard limits  yellow=safe bounds")
            ...
            # Trail XY (cyan)
            self._trail_xy = self._plot_xy.plot([], [], pen=pg.mkPen("c", width=2))
            ...
            # ---- Right: Front View (XZ) -------------------------------------
            self._plot_xz = pg.PlotWidget(title="Front View (XZ)  —  white=hard limits  yellow=safe bounds")
            ...
            # Trail XZ (cyan)
            self._trail_xz = self._plot_xz.plot([], [], pen=pg.mkPen("c", width=2))
            ...
            # Status bar
            self.statusBar().showMessage("Waiting for robot data ...")
